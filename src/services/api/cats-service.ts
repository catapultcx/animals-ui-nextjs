import { Cat } from "@/domain/cat";
import { BaseAPIService } from "./base-api-service";

interface GetCommand {
  id: string;
}

export class CatsService extends BaseAPIService {
  async get(params: GetCommand): Promise<Cat | null> {
    return await this._fetchGET(`${this.baseUrl}/cats/${params.id}`);
  }

  async all(): Promise<{ cats: Cat[] } | null> {
    return this._fetchGET(`${this.baseUrl}/cats`);
  }

  async getCatsByFilter(queryStrings: string): Promise<{ cats: Cat[] } | null> {
    return this._fetchGET(`${this.baseUrl}/cats?${queryStrings}`);
  }

  async create(cat: Cat): Promise<{ cats: Cat } | null> {
    return this._fetchPOST(`${this.baseUrl}/cats`, cat);
  }

  async update(cat: Cat): Promise<{ cats: Cat } | null> {
    return this._fetchPUT(`${this.baseUrl}/cats`, cat);
  }

  async delete(id: String): Promise<void | null> {
    return this._fetchDELETE(`${this.baseUrl}/cats/${id}`);
  }
}
